...
public class BlockchainIntegrationExample {
    ...
    public void processDonationWithBlockchain(Long donationId, BigDecimal amount, String donorEmail) {
        try {
            ...
            blockchainFuture.thenAccept(result -> {
                if (result != null && result.getTransactionHash() != null) {
                    log.info("Blockchain recording successful - TxHash: {}", result.getTransactionHash());
                    // Update database with transaction hash if needed
                    updateDonationWithTxHash(donationId, result.getTransactionHash());
                } else {
    ...
    public CompletableFuture<Void> recordMultipleDonationsBatch() {
        ...
        CompletableFuture<BlockchainService.BlockchainResult> donation1 = blockchainService.recordDonationOnBlockchain(
                "donor1@example.com", new BigDecimal("0.1"), "Education", "ORDER_1", System.currentTimeMillis() / 1000);
        
        CompletableFuture<BlockchainService.BlockchainResult> donation2 = blockchainService.recordDonationOnBlockchain(
                "donor2@example.com", new BigDecimal("0.2"), "Healthcare", "ORDER_2", System.currentTimeMillis() / 1000);
        
        CompletableFuture<BlockchainService.BlockchainResult> donation3 = blockchainService.recordDonationOnBlockchain(
    ...
    public void processDonationWithErrorHandling(Long donationId, BigDecimal amount, String donorEmail) {
        try {
            ...
            blockchainService.recordDonationOnBlockchain(
                ...
                if (result != null && result.getTransactionHash() != null) {
                    log.info("✅ Blockchain success: {}", result.getTransactionHash());
                } else {
