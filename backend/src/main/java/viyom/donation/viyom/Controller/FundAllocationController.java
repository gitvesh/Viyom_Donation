...
public class FundAllocationController {
    ...
    public ResponseEntity<?> allocateFunds(@RequestBody FundAllocationRequest request) {
        ...
        try {
            ...
            // Map to the new DTO for consistency and to avoid circular reference issues
            AllocationResponse response = AllocationResponse.builder()
                    ...
                    .allocationTxnHash(null) // Starts as pending
    ...
    public ResponseEntity<List<AllocationResponse>> getAllAllocations() {
        List<AllocationResponse> allocations = fundAllocationService.getAllAllocationsResponse();
        return ResponseEntity.ok(allocations);
    }

    @GetMapping("/fund-allocations")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<AllocationResponse>> getLegacyAllocations() {
    ...
    public ResponseEntity<Map<String, Object>> getLedgerSummary() {
        List<AllocationResponse> allAllocations = fundAllocationService.getAllAllocationsResponse();
        
        BigDecimal totalAllocated = allAllocations.stream()
                .map(AllocationResponse::getAmount)
